...
<div id="popup"
	class="fixed w-full h-full top-0 left-0 items-center flex justify-center hidden z-50 bg-black/60">
	<div
		...
		<div class="w-[90%] md:w-1/3 h-8 fixed rounded-tr-2xl rounded-tl-2xl">
			<div class="flex justify-end">
				<span onclick="closePopup()"
					class="text-2xl font-bold cursor-pointer mr-3">&times;</span>
			</div>
		</div>
		...
		<div class="my-4 flex items-center gap-3">
			...
			<form action="" method="post">
				<input type="hidden" name="action" value="delete_comment" /> <input
					type="hidden" name="articleId" value="${article.id}" /> <input
					type="hidden" name="pageNumber" value="${currentPage}" /> <input
...
<div class="w-[95%] md:w-4/5 mx-auto flex flex-col gap-2">
		...
		<div class="flex flex-col w-full shadow-xl rounded-xl border">
			<div class="w-full flex justify-between items-center">
				...
				<div class="p-2">
					...
					<!-- Update Button -->
					<form action="article" method="get" style="display: inline;"
						class="update-button">
					...
					<form action="article" method="post" style="display: inline;">
						...
						<input type="hidden" name="article_id" value="${article.id}" /> <input
							type="hidden" name="pageNumber" value="${currentPage}" />
	...
	<div class="flex flex-col gap-4 w-[95%] mx-auto md:mx-0 md:w-4/5">
		...
		<form action="" method="post">
			...
			<textarea name="content" cols="30" rows="5"
				class="w-full resize-none shadow-xl border-t-2 rounded-xl p-4"
				placeholder="Leave a comment!"></textarea>
			<input type="hidden" name="action" value="create_comment" /> <input
				type="hidden" name="articleId" value="${article.id}" /> <input
				type="hidden" name="pageNumber" value="${currentPage}" /> <input
	...
	<div id="comments"
		class="flex flex-col gap-4 w-[95%] mx-auto md:mx-0 md:w-4/5 mt-6 p-2 bg-[#f5f5f5] rounded-lg">
		...
		<c:choose>
							...
							<c:choose>
								<c:when test="${i == currentPage}">
									<li
										...
										<a class="page-link"
										href="article?article_id=${articleId}&page=${i}">${i}</a>
									</li>
								</c:when>
								<c:otherwise>
									<li
			...
			<c:otherwise>
				<div
					class="flex flex-col w-full shadow-md rounded-lg border-t-2 p-2 pl-4 text-center bg-gray-200">
					<p>No comments</p>
				</div>
			</c:otherwise>
		</c:choose>
...
<div id="updateArticleModal"
	...
	<div class="bg-white rounded-lg p-6 w-11/12 md:w-1/2">
		...
		<form action="" method="post">
			<input type="hidden" name="action" value="update_article" /> 
			<input
				type="hidden" name="article_id" value="${article.id}"
